...
import BUXWallContent from './BUXWallContent';
import * as THREE from 'three';

const BUXWall = ({ position, rotation, onCameraRotate }) => {
  const wallWidth = 9.5;
  const wallHeight = 7.5;

  const borderMaterial = new THREE.ShaderMaterial({
    uniforms: {
      color: { value: new THREE.Color('#00ffff') }, // Neon Blue
    },
    vertexShader: `
      varying vec2 vUv;
      void main() {
  ...
  return (
    <group position={position} rotation={rotation}>
      ...
      <group position={[0, 0, 0.01]}>
        <BUXWallContent onCameraRotate={onCameraRotate} />
      </group>
    </group>
  );
};

export default BUXWall;
