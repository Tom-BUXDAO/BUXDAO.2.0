...
import SpadesWallContent from './SpadesWallContent';
import * as THREE from 'three';

const SpadesWall = ({ position, rotation, onCameraRotate }) => {
  const wallWidth = 9.5;
  const wallHeight = 7.5;

  const borderMaterial = new THREE.ShaderMaterial({
    uniforms: {
      color: { value: new THREE.Color('#00ff00') }, // Neon Green
    },
    vertexShader: `
      varying vec2 vUv;
      void main() {
  ...
  return (
    <group position={position} rotation={rotation}>
      ...
      <group position={[0, 0, 0.01]}>
        <SpadesWallContent onCameraRotate={onCameraRotate} />
      </group>
    </group>
  );
};

export default SpadesWall;
