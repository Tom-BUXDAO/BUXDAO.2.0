...
import ShopWallContent from './ShopWallContent';
import * as THREE from 'three';

const ShopWall = ({ position, rotation, onCameraRotate }) => {
  const wallWidth = 9.5;
  const wallHeight = 7.5;

  const borderMaterial = new THREE.ShaderMaterial({
    uniforms: {
      color: { value: new THREE.Color('#ff00ff') }, // Neon Pink
    },
    vertexShader: `
      varying vec2 vUv;
      void main() {
  ...
  return (
    <group position={position} rotation={rotation}>
      ...
      <group position={[0, 0, 0.01]}>
        <ShopWallContent onCameraRotate={onCameraRotate} />
      </group>
    </group>
  );
};
